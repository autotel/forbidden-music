...
export const useSynthStore = defineStore("synthesizers", () => {
    ...
    const channels = ref<SynthChain[]>([]);
    const instancedSynths = ref<AdmissibleSynthType[]>([]);

    const addChannel = () => {
        const newChannel = new SynthChain();
        newChannel.destination = masterEffectsStore.myInput;
    ...
    const applyChainDefinition = (chain: SynthChain, definition: SynthChainDefinition) => {
        definition.forEach((chainStep, i) => {
            if (Array.isArray(chainStep)) {
                ...
                const newChain = new SynthChain();
                newChain.destination = masterEffectsStore.myInput;
