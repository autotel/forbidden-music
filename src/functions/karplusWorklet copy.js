...
class DelayLine extends SampleBySampleOperator {
    ...
    operation = (insample) => {
        ...
        while (memory.length > this.delaySamples) {
            ret += memory.pop() || 0;
        }
        // if(memory.length > this.delaySamples) {
        //     memory.splice(memory.length - this.delaySamples, memory.length);
        // }
        ...
        memory.unshift(ret * this.feedback);
        return ret;
    }
}
